...
namespace IsekaiMod.Utilities
{
	internal static class DescriptionTools
	{
		...
		private static readonly EncyclopediaEntry[] EncyclopediaEntries = new EncyclopediaEntry[61]
		{
			...
			{
				Entry = "Saving_Throw",
				Patterns = { "saving throws", "Saving Throw", "Reflex Save", "Will Save", "Fortitude Save" }
			},
			new EncyclopediaEntry
			{
			...
			new EncyclopediaEntry
			{
				...
				Patterns = { "Flat Footed AC", "Flat Footed Armor Class" }
			},
			new EncyclopediaEntry
			{
				Entry = "Flat_Footed",
				Patterns = { "Flat Footed", "Flat-Footed" }
			},
			new EncyclopediaEntry
			{
				Entry = "Armor_Class",
				Patterns = { "Armor Class", "AC" }
			},
			new EncyclopediaEntry
			{
			...
			{
				Entry = "CMB",
				Patterns = { "Combat Maneuver Bonus", "CMB" }
			},
			new EncyclopediaEntry
			{
				Entry = "CMD",
				Patterns = { "Combat Maneuver Defense", "CMD" }
			},
			new EncyclopediaEntry
			{
				Entry = "BAB",
				Patterns = { "Base Attack Bonus", "BAB" }
			},
			new EncyclopediaEntry
			{
			...
			{
				Entry = "NaturalAttack",
				Patterns = { "Natural Attacks?", "Natural Weapons?" }
			},
			new EncyclopediaEntry
			{
				Entry = "Attack_Of_Opportunity",
				Patterns = { "Attacks? Of Opportunity", "AoO" }
			},
			new EncyclopediaEntry
			{
				Entry = "Penalty",
			...
			{
				Entry = "Critical",
				Patterns = { "Critical Hit", "critical hits" }
			},
			new EncyclopediaEntry
			{
				Entry = "Fast_Healing",
			...
			{
				Entry = "Damage_Type",
				Patterns = { "Bludgeoning", "Piercing", "Slashing" }
			},
			new EncyclopediaEntry
			{
				Entry = "Damage",
				Patterns = { "damage", "damage rolls?" }
			},
			new EncyclopediaEntry
			{
				Entry = "Dice",
				Patterns = { "rolls?", "rolled" }
			}
		};
		...
		private static string ApplyTags(this string str, string from, EncyclopediaEntry entry)
		{
			...
			{
				str = new Regex(Regex.Escape(text).EnforceSolo().ExcludeTagged(), RegexOptions.IgnoreCase).Replace(str, entry.Tag(text), 1);
			}
			return str;
		}
