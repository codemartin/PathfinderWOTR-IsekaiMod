...
namespace IsekaiMod.Content.Features.IsekaiProtagonist.SpecialPower
{
	internal class IsekaiChannelPositiveEnergy
	{
		...
		{
			BlueprintAbility HarmAbility = Helpers.CreateBlueprint(Main.IsekaiContext, "IsekaiChannelPositiveEnergyAbility", delegate(BlueprintAbility bp)
			{
				bp.SetName(Main.IsekaiContext, "Channel Positive Energy - Damage Undead");
				bp.SetDescription(Main.IsekaiContext, "Channeling positive energy causes a burst that damages every undead creature in a 30-foot radius centered on the caster. The amount of damage inflicted is equal to 1d6 points of damage plus 1d6 points of damage for every two Isekai Protagonist levels beyond 1st (2d6 at 3rd, 3d6 at 5th, and so on). Creatures that take damage from channeled energy receive a Will save to halve the damage. The DC of this save is equal to 10 + 1/2 the Isekai Protagonist level + Charisma modifier.");
				((BlueprintUnitFact)bp).m_Icon = ChannelPositiveHarm.Icon;
			...
			BlueprintAbility HealAbility = Helpers.CreateBlueprint(Main.IsekaiContext, "IsekaiChannelPositiveHealAbility", delegate(BlueprintAbility bp)
			{
				bp.SetName(Main.IsekaiContext, "Channel Positive Energy - Heal Living");
				bp.SetDescription(Main.IsekaiContext, "Channeling positive energy causes a burst that heals every living creature in a 30-foot radius centered on the caster. The amount of damage healed is equal to 1d6 points of damage plus 1d6 points of damage for every two Isekai Protagonist levels beyond 1st (2d6 at 3rd, 3d6 at 5th, and so on).");
				((BlueprintUnitFact)bp).m_Icon = ChannelPositiveHeal.Icon;
				bp.LocalizedDuration = StaticReferences.Strings.Null;
				bp.LocalizedSavingThrow = StaticReferences.Strings.Null;
			...
			AddChannelEnergyPatchedComponents(HarmAbility, ((BlueprintScriptableObject)ChannelPositiveHarm).Components);
			AddChannelEnergyPatchedComponents(HealAbility, ((BlueprintScriptableObject)ChannelPositiveHeal).Components);
			BlueprintFeature feature = Helpers.CreateBlueprint(Main.IsekaiContext, "IsekaiChannelPositiveEnergyFeature", delegate(BlueprintFeature bp)
			{
				bp.SetName(Main.IsekaiContext, "Channel Positive Energy");
				bp.SetDescription(Main.IsekaiContext, "You gain the supernatural ability to channel positive energy like a cleric. You use your Isekai Protagonist level as your effective cleric level when channeling positive energy.  You can channel energy a number of times per day equal to 3 + your Charisma modifier.");
				((BlueprintUnitFact)bp).m_Icon = ChannelPositiveHeal.Icon;
