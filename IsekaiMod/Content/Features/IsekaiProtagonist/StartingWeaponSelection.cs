...
namespace IsekaiMod.Content.Features.IsekaiProtagonist
{
	internal class StartingWeaponSelection
	{
		...
		private static readonly Sprite Icon_StartSword = AssetLoader.LoadInternal(Main.IsekaiContext, "Features", "ICON_START_SWORD.png");

		private static readonly Sprite Icon_StartBlunt = AssetLoader.LoadInternal(Main.IsekaiContext, "Features", "ICON_START_BLUNT.png");

		private static readonly Sprite Icon_StartRanged = AssetLoader.LoadInternal(Main.IsekaiContext, "Features", "ICON_START_RANGED.png");

		public static void Add()
		{
			Helpers.CreateBlueprint(Main.IsekaiContext, "StartingWeaponSelection", delegate(BlueprintFeatureSelection bp)
			{
				bp.SetName(Main.IsekaiContext, "Starting Weapon");
				bp.SetDescription(Main.IsekaiContext, "At 1st level, you get to select a starting weapon. The weapon can be found in the Kenarbes festival chest.");
				bp.IgnorePrerequisites = true;
				((BlueprintUnitFact)bp).m_Icon = Icon_StartSword;
		...
		private static BlueprintFeatureReference CreateStartingWeapon(string name, string description, BlueprintItemWeapon weapon, Sprite icon)
		{
			...
			{
				LocalizedString localizedString = weapon?.m_Type?.Get()?.m_DefaultNameText;
				if (localizedString != null)
				{
					bp.SetName(localizedString);
				}
				else
				{
