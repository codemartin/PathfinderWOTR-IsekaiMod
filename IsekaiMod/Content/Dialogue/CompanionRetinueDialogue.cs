...
namespace IsekaiMod.Content.Dialogue
{
	internal class CompanionRetinueDialogue
	{
		...
		private static void AddRegillRetinueDialogue()
		{
			...
			BlueprintAnswer answer = TTCoreExtensions.CreateAnswer("IsekaiRetinueRegill", delegate(BlueprintAnswer bp)
			{
				bp.SetText(Main.IsekaiContext, "(Otherworld Retinue) \"Regill, how does fighting alongside someone whose soul hails from another reality affect your discipline? Does our retinue bond align with Hellknight doctrine?\"");
		...
		private static void AddArueshalaeRetinueDialogue()
		{
			...
			BlueprintAnswer answer = TTCoreExtensions.CreateAnswer("IsekaiRetinueArueshalae", delegate(BlueprintAnswer bp)
			{
				bp.SetText(Main.IsekaiContext, "(Otherworld Retinue) \"Arueshalae, what does our soul bond feel like to you? Can you sense the otherworld energies flowing through our retinue?\"");
				bp.NextCue = new CueSelection
				{
					Cues = new List<BlueprintCueBaseReference> { reply.ToReference<BlueprintCueBaseReference>() },
					Strategy = Strategy.First
		...
		private static void AddDaeranRetinueDialogue()
		{
			...
			BlueprintCue reply = TTCoreExtensions.CreateCue("IsekaiRetinueDaeranReply", delegate(BlueprintCue bp)
			{
				bp.SetText(Main.IsekaiContext, "{n}Daeran swirls his goblet with theatrical admiration, flashing a wicked, conspiratorial grin.{/n} \"Oh, it is simply magnificent, my dear Commander! Do you know how tiring it is to look over one's shoulder expecting assassins, Inquisitors, or otherworldly entities at every turn? And then you stroll in, radiating narrative immunity like cheap cologne!\"\n\n{n}He leans back against his cushions, basking in sheer amusement.{/n} \"Swords miss by inches, curses fizzle out, and every critical blow somehow feels like a grand spectacle orchestrated for our personal amusement. If the cosmos decided to make you the star of this absurd crusade, having VIP front-row seats in your retinue is the finest luxury I could ever buy. Do keep it up, darling, dying is so dreadfully pedestrian.\"");
				if (daeranUnit != null)
				{
					bp.Speaker = new DialogSpeaker
					{
			...
			BlueprintAnswer answer = TTCoreExtensions.CreateAnswer("IsekaiRetinueDaeran", delegate(BlueprintAnswer bp)
			{
				bp.SetText(Main.IsekaiContext, "(Otherworld Retinue) \"Daeran, you seem entirely too comfortable benefiting from otherworldly plot armor. Aren't you worried about the philosophical implications?\"");
				bp.NextCue = new CueSelection
				{
					Cues = new List<BlueprintCueBaseReference> { reply.ToReference<BlueprintCueBaseReference>() },
					Strategy = Strategy.First
		...
		private static void AddWenduagRetinueDialogue()
		{
			...
			BlueprintAnswer answer = TTCoreExtensions.CreateAnswer("IsekaiRetinueWenduag", delegate(BlueprintAnswer bp)
			{
				bp.SetText(Main.IsekaiContext, "(Otherworld Retinue) \"Wenduag, how do you feel about the instincts and authority channeled through our retinue bond?\"");
				bp.NextCue = new CueSelection
				{
					Cues = new List<BlueprintCueBaseReference> { reply.ToReference<BlueprintCueBaseReference>() },
					Strategy = Strategy.First
		...
		private static void AddSeelahRetinueDialogue()
		{
			...
			BlueprintAnswer answer = TTCoreExtensions.CreateAnswer("IsekaiRetinueSeelah", delegate(BlueprintAnswer bp)
			{
				bp.SetText(Main.IsekaiContext, "(Otherworld Retinue) \"Seelah, how does fighting alongside our retinue compare to serving in a standard paladin order?\"");
				bp.NextCue = new CueSelection
				{
					Cues = new List<BlueprintCueBaseReference> { reply.ToReference<BlueprintCueBaseReference>() },
					Strategy = Strategy.First
		...
		private static void AddCamelliaRetinueDialogue()
		{
			...
			BlueprintAnswer answer = TTCoreExtensions.CreateAnswer("IsekaiRetinueCamellia", delegate(BlueprintAnswer bp)
			{
				bp.SetText(Main.IsekaiContext, "(Otherworld Retinue) \"Camellia, what do your spirits have to say about the otherworldly power circulating through our retinue?\"");
				bp.NextCue = new CueSelection
				{
					Cues = new List<BlueprintCueBaseReference> { reply.ToReference<BlueprintCueBaseReference>() },
					Strategy = Strategy.First
		...
		private static void AddLannRetinueDialogue()
		{
			...
			BlueprintAnswer answer = TTCoreExtensions.CreateAnswer("IsekaiRetinueLann", delegate(BlueprintAnswer bp)
			{
				bp.SetText(Main.IsekaiContext, "(Otherworld Retinue) \"Lann, have you noticed how our retinue bond changes your archery and battlefield survivability?\"");
				bp.NextCue = new CueSelection
				{
					Cues = new List<BlueprintCueBaseReference> { reply.ToReference<BlueprintCueBaseReference>() },
					Strategy = Strategy.First
